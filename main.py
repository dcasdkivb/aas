        ...
        async with aiohttp.ClientSession() as session:
            async with session.post("https://0x0.st", data=data) as response:
                if response.status == 200:
                    ...
                    embed = discord.Embed(title="Shelby Music", description=description, color=color)
                    if is_nsfw:
                        embed.add_field(name="0x0 File URL", value=f"||{file_url}||", inline=False)
                    else:
                        embed.add_field(name="To download the music, follow the link below.", value=file_url, inline=True)
                        embed.set_image(url=attachment.url)
                    embed.set_footer(text=footer_text)
                    
                    view = discord.ui.View()
                    view.add_item(discord.ui.Button(label="Open File", url=file_url))
                    view.add_item(discord.ui.Button(label="Virus Check", url=f"https://www.virustotal.com/gui/home/search{hash_value}"))
